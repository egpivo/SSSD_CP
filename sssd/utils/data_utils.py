...
def vis_na(result, figsize_width, figsize_height):
    ...
    result = (
        result.reset_index()
    )  # result 的 index 不一定是從 0 開始，所以 reset，使 result.loc[0,'Date'] 可以使用
    no_day = int(result.shape[0] / 24)

    # create a day list
    start_date = pd.to_datetime(
        result.loc[0, "Date"], format="%Y%m%d"
    )  # 将初始日期转换为 Pandas 的 datetime 格式
...
def plot_impute_result(df_impute_stl, imputed_indices):
    ...
    plt.scatter(
        imputed_indices, df_impute_stl.loc[imputed_indices, "Load"], color="red"
    )
...
def pd_to_numpy(load, zones_num, days_window):
    ...
    for day in days:
        ...
        selected_data = load.loc[
            (load["Date"] >= start_date) & (load["Date"] < end_date), :
        ]
...
def z_normalization(data, days_normalized):
    ...
    mean = np.mean(data[:, 0 : (days_normalized * 24), :], axis=1).reshape(
        obs, 1, channel
    )
    std = np.std(data[:, 0 : (days_normalized * 24), :], axis=1).reshape(
        obs, 1, channel
    )
...
def range_normalization(data, days_normalized):
    ...
    min_value = np.min(data[:, 0 : (days_normalized * 24), :], axis=1).reshape(
        obs, 1, channel
    )
    max_value = np.max(data[:, 0 : (days_normalized * 24), :], axis=1).reshape(
        obs, 1, channel
    )
