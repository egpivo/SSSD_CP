...
def calc_diffusion_step_embedding(diffusion_steps, diffusion_step_embed_dim_in, device="cuda"):
    """
...
def sampling(net, size, diffusion_hyperparams, cond, mask, only_generate_missing=0, guidance_weight=0, device="cuda"):
    """
...
def get_mask_bm(sample, k):
    ...
    s_nan = random.choice(list_of_segments_index)
    for channel in range(mask.shape[1]):
        mask[:, channel][s_nan[0]:s_nan[-1] + 1] = 0 # mask[:,channel] = mask[:,channel,:] select one channel
...
def get_mask_forecast(sample, k):
    ...
    mask = torch.ones(sample.shape)
    s_nan = torch.tensor(range(mask.shape[0]-k, mask.shape[0]))
    for channel in range(mask.shape[1]):
        mask[:, channel][s_nan[0]:s_nan[-1] + 1] = 0 # mask[:,channel] = mask[:,channel,:] select one channel
