...
class S4Layer(nn.Module):
    ...
    def __init__(self, features, lmax, N=64, dropout=0.0, layer_norm=True):
        super().__init__()
        self.s4_layer = S4(d_model=features, d_state=N, l_max=251, bidirectional=True)
...
def evaluate(model, test_loader, nsample=100, scaler=1, mean_scaler=0, path_save=""):
    with torch.no_grad():
        ...
        with tqdm(test_loader, mininterval=5.0, maxinterval=5.0) as it:
            ...
            with open(
                f"{path_save}generated_outputs_nsample" + str(nsample) + ".pk", "wb"
            ) as f:
...
def Conv1d_with_init(input_channels, output_channels, kernel_size):
    layer = nn.Conv1d(input_channels, output_channels, kernel_size)
...
def get_dataloader_train_impute(
    series,
    batch_size=4,
    ...
    valid_dataset = Custom_Train_Dataset(
        series=series,
        use_index_list=v_i,
        missing_ratio_or_k=missing_ratio_or_k,
        masking=masking,
    ...
    test_dataset = Custom_Train_Dataset(
        series=series,
        use_index_list=te_i,
        missing_ratio_or_k=missing_ratio_or_k,
        masking=masking,
...
def get_dataloader_impute(series, mask, batch_size=4, len_dataset=100):
    indlist = np.arange(len_dataset)
    impute_dataset = Custom_Impute_Dataset(
        series=series, use_index_list=indlist, mask=mask
    )
...
class CSDIS4Imputer:
    ...
    def load_weights(self, path_load_model="", path_config=""):
    ...
    def impute(
        ...
        test_loader = get_dataloader_impute(
            series=self.series_impute,
        ...
        try:
            ...
            model.load_state_dict(
                torch.load(self.path_load_model_dic, map_location=torch.device("cpu"))
            )

        imputations = evaluate(
            model=model,
            test_loader=test_loader,
            nsample=n_samples,
            scaler=1,
        ...
        for original_sample, single_n_samples in zip(
            sample.detach().cpu().numpy(), imputations
