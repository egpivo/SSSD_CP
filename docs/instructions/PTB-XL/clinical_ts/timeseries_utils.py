...
def butter_filter(lowcut=10, highcut=20, fs=50, order=5, btype="band"):
    ...
    sos = butter(
        order,
        [low, high] if btype == "band" else (low if btype == "low" else high),
        analog=False,
        btype=btype,
        output="sos",
    )
...
def load_dataset(target_root, filename_postfix="", df_mapped=True):
    ...
    if df_mapped:
        df = pickle.load(
            open(target_root / ("df_memmap" + filename_postfix + ".pkl"), "rb")
        )
    else:
        df = pickle.load(open(target_root / ("df" + filename_postfix + ".pkl"), "rb"))

    if (
        target_root / ("lbl_itos" + filename_postfix + ".pkl")
    ).exists():  # dict as pickle
...
def dataset_add_length_col(df, col="data", data_folder=None):
    ...
    df[col + "_length"] = df[col].apply(
        lambda x: len(
            np.load(x if data_folder is None else data_folder / x, allow_pickle=True)
        )
    )
...
def dataset_add_labels_col(df, col="label", data_folder=None):
    ...
    df[col + "_labels"] = df[col].apply(
        lambda x: list(
            np.unique(
                np.load(
                    x if data_folder is None else data_folder / x, allow_pickle=True
                )
            )
        )
    )
    ...
    if simple:
        ...
        def combine_two_means_vars(x1, x2):
            ...
            var = (
                var1 * n1 / (n1 + n2)
                + var2 * n2 / (n1 + n2)
                + n1 * n2 / (n1 + n2) / (n1 + n2) * np.power(mean1 - mean2, 2)
            )
...
def npys_to_memmap_batched(
    npys, target_filename, max_len=0, delete_npys=True, batch_length=900000
):
    memmap = None
    start = np.array(
        [0]
    )  # start_idx in current memmap file (always already the next start- delete last token in the end)
    ...
    for idx, npy in tqdm(list(enumerate(npys))):
        ...
        if idx == len(npys) - 1 or np.sum(data_lengths) > batch_length:  # flush
            ...
            if memmap is None or (
                max_len > 0 and start[-1] > max_len
            ):  # new memmap file has to be created
                if max_len > 0:
                    filenames.append(
                        target_filename.parent
                        / (target_filename.stem + "_" + str(len(filenames)) + ".npy")
                    )
                else:
                    filenames.append(target_filename)

                shape.append(
                    [np.sum(data_lengths)] + [l for l in data.shape[1:]]
                )  # insert present shape
                ...
                memmap = np.memmap(
                    filenames[-1], dtype=data.dtype, mode="w+", shape=data.shape
                )
            else:
                ...
                memmap = np.memmap(
                    filenames[-1], dtype=data.dtype, mode="r+", shape=tuple(shape[-1])
                )

            # store mapping memmap_id to memmap_file_id
            file_idx = np.concatenate(
                [file_idx, [(len(filenames) - 1)] * len(data_lengths)]
            )
            # insert the actual data
            memmap[
                start[-len(data_lengths) - 1] : start[-len(data_lengths) - 1]
                + len(data)
            ] = data[:]
            memmap.flush()
            dtype = data.dtype
    ...
    # save metadata
    np.savez(
        target_filename.parent / (target_filename.stem + "_meta.npz"),
        start=start,
        length=length,
        shape=shape,
        file_idx=file_idx,
...
def reformat_as_memmap(
    ...
    for _, row in df.iterrows():
        npys_data.append(
            data_folder / row[col_data] if data_folder is not None else row[col_data]
        )
        if annotation:
            npys_label.append(
                data_folder / row[col_label]
                if data_folder is not None
                else row[col_label]
            )
    if batch_length == 0:
        npys_to_memmap(
            npys_data, target_filename, max_len=max_len, delete_npys=delete_npys
        )
    else:
        npys_to_memmap_batched(
            npys_data,
    ...
    if annotation:
        if batch_length == 0:
            npys_to_memmap(
                npys_label,
                target_filename.parent / (target_filename.stem + "_label.npy"),
                max_len=max_len,
                delete_npys=delete_npys,
            )
        else:
    ...
    df_mapped.to_pickle(
        target_filename.parent / ("df_" + target_filename.stem + ".pkl")
    )
    return df_mapped
...
class ConcatDatasetTimeseriesDatasetCrops(torch.utils.data.ConcatDataset):
    ...
    def __init__(self, datasets):
        ...
        for dataset_idx, ds in enumerate(self.datasets):
            ...
            remap_dict = {
                x: j
                + (self.cumulative_sizes[dataset_idx - 1] if dataset_idx > 0 else 0)
                for j, x in enumerate(np.unique(idmap))
            }
...
class TimeseriesDatasetCrops(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if self.timeseries_df_data.dtype not in [np.int16, np.int32, np.int64]:
            assert (
                memmap_filename is None and npy_data is None
            )  # only for filenames in mode files
            self.timeseries_df_data = np.array(df["data"].astype(str)).astype(
                np.string_
            )
        ...
        if col_lbl is None:  # use dummy labels
            ...
            if isinstance(df[col_lbl].iloc[0], list) or isinstance(
                df[col_lbl].iloc[0], np.ndarray
            ...
            if self.timeseries_df_label.dtype not in [
                np.int16,
                np.int32,
                np.int64,
                np.float32,
                ...
                assert (
                    annotation and memmap_filename is None and npy_data is None
                )  # only for filenames in mode files
                self.timeseries_df_label = np.array(
                    df[col_lbl].apply(lambda x: str(x))
                ).astype(np.string_)
        ...
        if isinstance(self.transforms, list) or isinstance(self.transforms, np.ndarray):
            print(
                "Warning: the use of lists as arguments for transforms is discouraged"
            )
        ...
        if memmap_filename is not None:
            self.memmap_meta_filename = memmap_filename.parent / (
                memmap_filename.stem + "_meta.npz"
            )
            self.mode = "memmap"
            ...
            self.memmap_filenames = np.array(memmap_meta["filenames"]).astype(
                np.string_
            )  # save as byte to avoid issue with mp
            if annotation:
                memmap_meta_label = np.load(
                    self.memmap_meta_filename.parent
                    / (
                        "_".join(self.memmap_meta_filename.stem.split("_")[:-1])
                        + "_label_meta.npz"
                    ),
                    allow_pickle=True,
                )
                self.memmap_shape_label = memmap_meta_label["shape"]
                self.memmap_filenames_label = np.array(
                    memmap_meta_label["filenames"]
                ).astype(np.string_)
            ...
            if annotation:
                self.npy_data_label = np.load(
                    npy_data.parent / (npy_data.stem + "_label.npy"), allow_pickle=True
                )
            ...
            if chunk_length == 0:  # do not split
                ...
                idx_start = list(
                    range(
                        start_idx,
                        data_length,
                        chunk_length if stride is None else stride,
                    )
                )
    ...
    def _getitem(self, idx, start_idx_rel):
        ...
        if self.mode == "files":  # from separate files
            data_filename = str(
                self.timeseries_df_data[df_idx], encoding="utf-8"
            )  # todo: fix potential issues here
            if self.data_folder is not None:
                data_filename = self.data_folder / data_filename
            data = np.load(data_filename, allow_pickle=True)[
                start_idx_crop:end_idx_crop
            ...
            if self.annotation is True:
                ...
                label = self.timeseries_df_label[
                    df_idx
                ]  # input type has to be adjusted in the dataframe
            ...
            mem_file = np.memmap(
                self.memmap_meta_filename.parent / mem_filename,
                self.memmap_dtype,
                mode="r",
                shape=tuple(self.memmap_shape[memmap_file_idx]),
            )
            data = np.copy(
                mem_file[idx_offset + start_idx_crop : idx_offset + end_idx_crop]
            )
            del mem_file
            # print(mem_file[idx_offset + start_idx_crop: idx_offset + end_idx_crop])
            if self.annotation:
                mem_filename_label = str(
                    self.memmap_filenames_label[memmap_file_idx], encoding="utf-8"
                )
                mem_file_label = np.memmap(
                    self.memmap_meta_filename.parent / mem_filename_label,
                    self.memmap_dtype_label,
                    mode="r",
                ...
                label = np.copy(
                    mem_file_label[
                        idx_offset + start_idx_crop : idx_offset + end_idx_crop
                    ]
                )
                del mem_file_label
            else:
        ...
        sample = (
            data,
            label,
            self.timeseries_df_static[df_idx] if self.static else None,
        )
        ...
        ):  # single transform e.g. from torchvision.transforms.Compose
    ...
    def get_sampling_weights(
        self, class_weight_dict, length_weighting=False, timeseries_df_group_by_col=None
    ):
        """
        ...
        length_per_class = {}
        length_per_group = {}
        for iw, (i, s, e) in enumerate(
            zip(self.df_idx_mapping, self.start_idx_mapping, self.end_idx_mapping)
        ):
        ...
        if (
            length_weighting
        ):  # need second pass to properly take into account the total length per class
            for iw, (i, s, e) in enumerate(
                zip(self.df_idx_mapping, self.start_idx_mapping, self.end_idx_mapping)
            ):
                label = self.timeseries_df_label[i]
    ...
    def __call__(self, sample):
        if self.scale == 0:
            ...
            data = data + np.reshape(
                np.array(
                    [random.gauss(0, self.scale) for _ in range(np.prod(data.shape))]
                ),
                data.shape,
    ...
    def __call__(self, sample):
        if self.scale == 1:
            ...
            data = transform.resize(
                data, (timesteps_new, data.shape[1]), order=self.interpolation_order
            ).astype(np.float32)
            return data, label, static
...
class Normalize(object):
    ...
    def __init__(self, stats_mean, stats_std, input=True, channels=[]):
        self.stats_mean = (
            stats_mean.astype(np.float32) if stats_mean is not None else None
        )
        self.stats_std = (
            stats_std.astype(np.float32) + 1e-8 if stats_std is not None else None
        )
        self.input = input
        if len(channels) > 0:
...
class NormalizeBatch(object):
    ...
    def __call__(self, sample):
        ...
        batch_mean = np.mean(
            ...
            else tuple(range(0, len(data.shape) - 1)),
        )
        batch_std = (
            np.std(
                data,
...
def aggregate_predictions(
    preds, targs=None, idmap=None, aggregate_fn=np.mean, verbose=False
):
    """
