...
__all__ = [
    ...
    "prepare_data_icbeb",
    "prepare_data_ptb_xl",
    "map_and_filter_labels",
    "thew_to_np",
    "reformat_data_thew",
    "prepare_data_thew",
    "prepare_data_cinc",
    "prepare_data_chapman",
    "prepare_data_ribeiro_test",
    "prepare_data_ludb",
    "prepare_data_ribeiro_full",
    "prepare_mitdb",
    "prepare_afdb",
    "prepare_ltafdb",
    "prepare_vfdb",
    "prepare_cpsc21",
    "get_data_annot_stats",
]
...
def get_available_channels(channel_labels, channel_stoi):
    if channel_stoi is None:
        ...
        return sorted(
            [channel_stoi[c] for c in channel_labels if c in channel_stoi.keys()]
        )
...
def get_filename_out(filename_in, target_folder=None, suffix=""):
    if target_folder is None:
        ...
        if "train" in filename_in.parts:
            ...
            "eval" in filename_in.parts
            or "dev_test" in filename_in.parts
            or "valid" in filename_in.parts
            or "valtest" in filename_in.parts
        ):
...
def reformat_data_ptb(
    ...
    for filename_in in tqdm(datafiles):
        filename_out, filename_out_relative = get_filename_out(
            filename_in, target_folder
        )
        ...
        diagnosis2 = [
            l.strip()
            for l in metadata[7].split(",")
            if l.strip() not in ["no", "unknown", "n/a"]
        ]
        diagnosis2 = [
            l.replace("Typ IIa", "Type IIa")
            .replace("Hypertension", "hypertension")
            .replace(" (intraventricular gradient 100-160mmHg)", "")
            .replace("Fibrillation", "fibrillation")
            .replace(
                "Recurrent ventricular tachycardias",
                "Recurrent ventricular tachycardia",
            )
            .replace("Thyriodectomy", "Thyroidectomy")
            .replace(
                "Dilated Cardiomyopathy secondary to coronary artery disease. Mitral regurgitation (grade 2)",
        ...
        diagnosis2 = [
            l.replace("Hyperlipoproteinemia Type IIa", "Hyperlipoproteinemia")
            .replace("Hyperlipoproteinemia Type IV", "Hyperlipoproteinemia")
            .replace("Hyperlipoproteinemia Type IIb", "Hyperlipoproteinemia")
            for l in diagnosis2
        ]  # clean up
        ...
        row = {
            "data": filename_out_relative,
            "NYISO": "PTB-XL",
            "channels_available": channels_available,
            "patient": "PTB_" + patient,
            "ecg": "PTB_" + ecg,
            "age": age,
            "label_age": label_age,
            "sex": sex,
            ...
            "infarction_localization": metadata[5],
            "diagnosis2": diagnosis2,
            "diagnosis12": diagnosis12,
            "smoker": smoker,
            "label_smoker": label_smoker,
            "infarction_date": metadata[10],
            "catheterization_date": metadata[14],
        }
        # add hemodynamics
    ...
    if lbl_itos is not None:
        ...
        df_all["label"] = df_all.diagnosis.apply(
            lambda x: (lbl_stoi[x] if x in lbl_stoi.keys() else len(lbl_itos))
        )  # map everything that cannot be found in the dictionary to last entry
    else:
...
def _ptb_cleanup_hemodynamics(df_ptb):
    ...
    def _reformat_col(df, col_in, col_out1, col_out2=None):
        df[col_out1] = df[col_in].apply(
            lambda x: _extract_numeric(x, 0 if col_out2 is None else 1)
        )
    ...
    df_ptb["Cardiac index (at rest)"] = df_ptb["Cardiac index (at rest)"].apply(
        lambda x: x.replace(",", ".")
    )
    _reformat_col(df_ptb, "Cardiac index (at rest)", "cardiac_index_at_rest")
    df_ptb["Cardiac index (load)"] = df_ptb["Cardiac index (load)"].apply(
        lambda x: x.replace(",", ".")
    )
    _reformat_col(df_ptb, "Cardiac index (load)", "cardiac_index_load")
    df_ptb["Cardiac output (at rest)"] = df_ptb["Cardiac output (at rest)"].apply(
        lambda x: x.replace(",", ".")
    )
    _reformat_col(df_ptb, "Cardiac output (at rest)", "cardiac_output_at_rest")
    df_ptb["Cardiac output (load)"] = df_ptb["Cardiac output (load)"].apply(
        lambda x: x.replace(",", ".")
    )
    ...
    df_ptb["Peripheral blood Pressure (syst/diast)"] = df_ptb[
        "Peripheral blood Pressure (syst/diast)"
    ].apply(lambda x: x.replace("120 /70", "120/70"))
    _reformat_col(
        df_ptb,
        "Peripheral blood Pressure (syst/diast)",
        "peripheral_bp_syst",
    ...
    _reformat_col(
        ...
        "pulmonary_ap_at_rest_mean",
    )
    _reformat_col(
        df_ptb,
        "Pulmonary artery pressure (at rest) (syst/diast)",
        "pulmonary_ap_at_rest_syst",
        "pulmonary_ap_at_rest_diast",
    )
    _reformat_col(
        df_ptb, "Pulmonary artery pressure (laod) (mean)", "pulmonary_load_mean"
    )
    _reformat_col(
        df_ptb,
        "Pulmonary artery pressure (laod) (syst/diast)",
        "pulmonary_ap_load_syst",
        "pulmonary_ap_load_diast",
    )
    _reformat_col(
        df_ptb, "Pulmonary capillary wedge pressure (at rest)", "pulmonary_cwp_at_rest"
    )
    _reformat_col(
        df_ptb, "Pulmonary capillary wedge pressure (load)", "pulmonary_cwp_load"
    )

    df_ptb["Left ventricular enddiastolic pressure"] = df_ptb[
        "Left ventricular enddiastolic pressure"
    ].apply(
        lambda x: x[-8:].strip()
    )  # only post value
    _reformat_col(
        df_ptb, "Left ventricular enddiastolic pressure", "left_ventricular_ed_pressure"
    )

    _reformat_col(
        df_ptb, "Stroke volume index (at rest)", "stroke_volume_index_at_rest"
    )
    df_ptb["Stroke volume index (load)"] = df_ptb["Stroke volume index (load)"].apply(
        lambda x: x.replace(",", ".")
    )
    _reformat_col(df_ptb, "Stroke volume index (load)", "stroke_volume_index_load")


def prepare_data_ptb(
    data_folder,
    target_folder=None,
    channel_stoi=channel_stoi_default,
    channels=12,
    ...
    cv_random_state=42,
    recreate_data=False,
    ptb_permanently_remove_diagnosis_na=True,
    ptb_permanently_remove_multiple_ecgs=False,
    ptb_select_only_first_mi=True,
    ptb_remove_unknown_loc_mi=True,
    lbl_itos_ptb_in=["Healthy control", "Myocardial infarction"],
    lbl_itos_ptb_regression=["pulmonary_ap_at_rest_mean"],
    lbl_itos_ptb_all=["Diabetes mellitus"],
):
    ...
    if recreate_data:
        ...
        # reformat data
        df_ptb, lbl_itos_ptb = reformat_data_ptb(
            datafiles=datafiles_ptb,
            target_fs=target_fs,
            channels=channels,
            lbl_itos=lbl_itos_ptb_in,
            target_folder=target_folder,
        )

        df_ptb["infarction_localization_mapped"] = df_ptb[
            "infarction_localization"
        ].apply(lambda x: int_to_str[diagnosis_to_int_w_subdiagnosis[x]])
        df_ptb["diagnosis_extended"] = df_ptb.apply(
            lambda row: row["diagnosis"] + " " + row["infarction_localization_mapped"],
            axis=1,
        )
        # TODO move all this stuff into reformat_data_ptb
        # set regression label(s)
        df_ptb["label_regression"] = df_ptb.apply(
            lambda row: row[lbl_itos_ptb_regression[0]]
            if len(lbl_itos_ptb_regression) == 1
            else [row[regression_col] for regression_col in lbl_itos_ptb_regression],
            axis=1,
        )
        ...
        df_ptb["label_multi_all"] = df_ptb.diagnosis12.apply(
            lambda x: extract_multilabel(x, lbl_itos_ptb_all)
            if len(lbl_itos_ptb_all) > 1
            else int(lbl_itos_ptb_all[0] in x)
        )
        ...
        if ptb_permanently_remove_multiple_ecgs:
            ...
            df_ptb["drop"] = df_ptb.ecg.isin(
                df_ptb[
                    (df_ptb.diagnosis == "Myocardial infarction")
                    & (df_ptb.first_ecg == False)
                ].ecg
            )
        ...
        if ptb_select_only_first_mi:
            ...
            df_ptb2["first_ecg"] = df_ptb2.ecg.isin(
                df_ptb2.groupby("patient").first().ecg
            )
            df_ptb2["drop"] = df_ptb2.ecg.isin(
                df_ptb2[
                    (df_ptb2.diagnosis == "Myocardial infarction")
                    & (df_ptb2.first_ecg == False)
                ].ecg
            )
            df_ptb2 = (
                df_ptb2[df_ptb2["drop"] == False].reset_index().drop("index", axis=1)
            )
        ...
        if ptb_remove_unknown_loc_mi:
            df_ptb2 = df_ptb2[
                ~(
                    (df_ptb2.diagnosis == "Myocardial infarction")
                    & (df_ptb2.infarction_localization == "")
                )
            ]
        ...
        splits_patients = get_stratified_kfolds(
            np.array(label_patients), n_splits=strat_folds, random_state=cv_random_state
        )
        df_ptb["strat_fold"] = -1
        ...
        df_ptb_regression = df_ptb[
            df_ptb.label_regression.apply(lambda x: np.isfinite(x))
        ].copy()
        label_patients = df_ptb_regression.groupby("patient")[
            "diagnosis_extended"
        ].first()
        splits_patients = get_stratified_kfolds(
            np.array(label_patients), n_splits=strat_folds, random_state=cv_random_state
        ...
        for i, split in enumerate(splits_patients):
            ...
            ecgs_valid = np.array(
                df_ptb_regression[df_ptb_regression.patient.isin(patients_valid)]["ecg"]
            )
        ...
        for i, split in enumerate(splits_patients):
            ...
            ecgs_valid = np.array(
                df_ptb_smoker[df_ptb_smoker.patient.isin(patients_valid)]["ecg"]
            )
        ...
        for i, split in enumerate(splits_patients):
            ...
            ecgs_valid = np.array(
                df_ptb_age[df_ptb_age.patient.isin(patients_valid)]["ecg"]
            )
        ...
        for i, split in enumerate(splits_patients):
            ...
            ecgs_valid = np.array(
                df_ptb_sex[df_ptb_sex.patient.isin(patients_valid)]["ecg"]
            )
        ...
        for i in range(len(evallist)):
            df_ptb.loc[
                df_ptb.ecg.isin(["PTB_" + x for x in indexlst[evallist[i]][:, 1]]),
                "strat_fold_old",
            ] = i
        ...
        for i in range(strat_folds):
            valid_ecgs = np.array(
                df_ptb[(df_ptb.strat_fold >= 0) & (df_ptb.strat_fold == i)].ecg.apply(
                    lambda x: x[4:]
                )
            )
            train_ecgs = np.array(
        ...
        df_ptb, lbl_itos_ptb, mean_ptb, std_ptb = load_dataset(
            target_root_ptb, df_mapped=False
        )
...
def reformat_data_icbeb(
    ...
    labels.label1 = labels.label1.apply(
        lambda x: int(x) - 1 if not np.isnan(x) else np.nan
    )
    labels.label2 = labels.label2.apply(
        lambda x: int(x) - 1 if not np.isnan(x) else np.nan
    )
    labels.label3 = labels.label3.apply(
        lambda x: int(x) - 1 if not np.isnan(x) else np.nan
    )
    # data
    rows = []
    for d in tqdm(datafiles):
        sex, age, channels_available, filename_out, filename_out_relative = mat_to_np(
            filename_in=d,
            target_fs=target_fs,
            channels=channels,
            channel_stoi=channel_stoi,
        ...
        rows.append(
            {
                ...
                "NYISO": "ICBEB2018",
                "data": filename_out_relative,
                "age": (np.nan if np.isnan(age) else int(age)),
                "sex": sex.lower(),
                "channels_available": channels_available,
            }
        )
    ...
    df["label"] = df["label1"]
    df["has_label2"] = ~pd.isna(
        df["label2"]
    )  # i.e. multilabeled label 3 will only be set if label 2 is
...
def prepare_data_icbeb(
    ...
    if recreate_data:
        ...
        # reformat data
        df_icbeb, lbl_itos_icbeb = reformat_data_icbeb(
            datafiles_icbeb,
            labelfile_icbeb,
            channel_stoi=channel_stoi,
            target_fs=target_fs,
        ...
        # remove valid data
        df_icbeb["set"] = df_icbeb.data.apply(
            lambda x: (0 if "train" in x.parts else 1)
        )
        if discard_valtest is True:
            df_icbeb = df_icbeb[
                df_icbeb["set"] == 0
            ].reset_index()  # only train set has label
        ...
        stratified_ids = stratify(
            list(df_icbeb["label_multi"]),
            range(len(lbl_itos_icbeb)),
            [1.0 / strat_folds] * strat_folds,
        )
        ...
        df_icbeb, lbl_itos_icbeb, mean_icbeb, std_icbeb = load_dataset(
            target_root_icbeb, df_mapped=False
        )
...
def prepare_data_ptb_xl(
    ...
    if recreate_data is True:
        ...
        df_ptb_xl["label_diag_subclass"] = df_ptb_xl.label_diag.apply(
            lambda x: [
                diag_subclass_mapping[y] for y in x if y in diag_subclass_mapping
            ]
        )
        ...
        df_ptb_xl, lbl_itos_ptb_xl = map_and_filter_labels(
            df_ptb_xl,
            min_cnt=min_cnt,
            lbl_cols=[
                "label_all",
        ...
        for id, row in tqdm(list(df_ptb_xl.iterrows())):
            ...
            )  # data_path/row["filename_lr"] if target_fs<=100 else data_path/row["filename_hr"]
        ...
        # save
        save_dataset(
            df_ptb_xl, lbl_itos_ptb_xl, mean_ptb_xl, std_ptb_xl, target_root_ptb_xl
        )
    else:
...
def map_and_filter_labels(df, min_cnt, lbl_cols):
    ...
    def select_labels(labels, min_cnt=10):
        lbl, cnt = np.unique(
            [item for sublist in list(labels) for item in sublist], return_counts=True
        )
    ...
    for selection in lbl_cols:
        if min_cnt > 0:
                ...
                sorted(
                    list(
                        set(
                            [
                                x
                                for sublist in df_ptb_xl[selection + "_filtered"]
                                for x in sublist
                            ]
                        )
                    )
            ...
            lbl_stoi = {
                s: i for i, s in enumerate(lbl_itos_ptb_xl[selection + "_filtered"])
            }
            df_ptb_xl[selection + "_filtered_numeric"] = df_ptb_xl[
                selection + "_filtered"
            ].apply(lambda x: [lbl_stoi[y] for y in x])
        ...
        df_ptb_xl[selection + "_numeric"] = df_ptb_xl[selection].apply(
...
def thew_to_np(
    ...
    metadata = {
        "id": x.id,
        "length": x.ecg_size,
        "record_time": str(x.record_date) + " " + str(x.start_time),
        "channel_labels": [xx.spec_str().lower() for xx in x.lead],
    }
    metadata["channels_available"] = get_available_channels(
        metadata["channel_labels"], channel_stoi
    )
...
def reformat_data_thew(
    ...
    df_clinical = pd.read_excel(
        data_path / "clinicalData" / "E-HOL-12-1172-012.xls", sheet_name=1
    )
    ...
    for file in tqdm(list(data_path.glob("**/*.ecg"))):
        metadata, filename_out, filename_out_relative = thew_to_np(
            file,
            target_fs=target_fs,
            channels=channels,
        ...
        res.append(
            {
                "ecg_id": file.stem.replace("S-", "").replace("S", "").lstrip(),
                "patient_id": int(
                    file.stem.replace("S-", "").replace("S", "").lstrip()[:4]
                ),
                "data": filename_out_relative,
                "length": metadata["length"],
                "channels_available": metadata["channels_available"],
            }
        )
    df = pd.DataFrame(res)
    ...
    if len(df_error) > 0:
        print(
            "Skipping the following ECG files that could not be matched to diagnoses:",
            df_error,
        )
...
def prepare_data_thew(
    ...
    if recreate_data:
        # reformat data
        df_thew, lbl_itos_thew = reformat_data_thew(
        ...
        stratified_patient_ids = [
            [patients_ids[i] for i in fold] for fold in stratified_ids
        ]
        ...
        df_thew, lbl_itos_thew, mean_thew, std_thew = load_dataset(
            target_root_thew, df_mapped=False
        )
    return df_thew, lbl_itos_thew, mean_thew, std_thew


# Cell
def prepare_data_cinc(
    data_path,
    min_cnt=10,
    datasets=None,
    target_fs=100,
    ...
    if recreate_data is True:
        dx_meta = pd.concat(
            [
                pd.read_csv(data_path / "dx_mapping_scored.csv"),
                pd.read_csv(data_path / "dx_mapping_unscored.csv"),
            ],
            sort=True,
        )
        dx_mapping_snomed_abbrev = {
            a: b
            for [a, b] in list(
                dx_meta.apply(
                    lambda row: [row["SNOMED CT Code"], row["Abbreviation"]], axis=1
                )
            )
        }
        ...
        for filename in tqdm(list(data_path.glob("**/*.hea"))):
            ...
            for l in header["comments"]:
                ...
                if l.startswith("Dx:"):
                    labels = [
                        dx_mapping_snomed_abbrev[int(x)] for x in arrs[1].split(",")
                    ]
            ...
            metadata.append(
                {
                    "data": Path(filename.stem + ".npy"),
                    "label": labels,
                    "sex": sex,
                    "age": age,
                    "NYISO": "cinc_" + filename.parts[-2],
                }
            )
        ...
        for ds in np.unique(df["NYISO"]):
            ...
            lbl_itosx = np.unique(
                [item for sublist in list(dfx.label) for item in sublist]
            )
            stratified_ids = stratify(
                list(dfx["label"]), lbl_itosx, [1.0 / strat_folds] * strat_folds
            )
...
def prepare_data_chapman(
    ...
    if recreate_data is True:
        ...
        df["label_condition"] = df.Beat.apply(
            lambda x: [y for y in x.split(" ") if x != "NONE"]
        )
        df["label_rhythm"] = df.Rhythm.apply(lambda x: x.split(" "))
        df["label_all"] = df.apply(
            lambda row: row["label_condition"] + row["label_rhythm"], axis=1
        )
        df["sex"] = df.Gender.apply(lambda x: x.lower())
        df["age"] = df.PatientAge
        df.drop(
            ["Gender", "PatientAge", "Rhythm", "Beat", "FileName"], inplace=True, axis=1
        )
        ...
        df, lbl_itos = map_and_filter_labels(
            df,
            min_cnt=min_cnt,
            lbl_cols=["label_all", "label_condition", "label_rhythm"],
        )
        ...
        for id, row in tqdm(list(df.iterrows())):
            ...
            df_tmp = pd.read_csv(
                data_path
                / ("ECGDataDenoised" if denoised else "ECGData")
                / (row["id"] + ".csv")
            )
...
def prepare_data_ribeiro_test(
    ...
    if recreate_data is True:
        ...
        df_cardiology_residents = pd.read_csv(
            data_path / "annotations" / "cardiology_residents.csv"
        )
        df_dnn = pd.read_csv(data_path / "annotations" / "dnn.csv")
        df_emergency_residents = pd.read_csv(
            data_path / "annotations" / "emergency_residents.csv"
        )
        df_medical_students = pd.read_csv(
            data_path / "annotations" / "medical_students.csv"
        )
        df_attributes = pd.read_csv(data_path / "attributes.csv")
        sex_map = {"M": "male", "F": "female"}
        df_attributes.sex = df_attributes.sex.apply(lambda x: sex_map[x])

        def reformat_predictions(
            df,
            colname_txt="label_txt",
            colname_num="label",
            lbl_itos=["1AVB", "RBBB", "LBBB", "SBRAD", "AFIB", "STACH"],
        ):
            lbl_stoi = {s: i for i, s in enumerate(lbl_itos)}
            df[colname_txt] = df.apply(
                lambda row: ("1AVB " if row["1dAVb"] else "")
                + ("RBBB " if row["RBBB"] else "")
                + ("LBBB " if row["LBBB"] else "")
                + ("SBRAD " if row["SB"] else "")
                + ("AFIB " if row["AF"] else "")
                + ("STACH " if row["ST"] else ""),
                axis=1,
            )
            df[colname_txt] = df[colname_txt].apply(
                lambda x: [y for y in x.strip().split(" ") if y != ""]
            )
            df[colname_num] = df[colname_txt].apply(
                lambda x: [lbl_stoi[y] for y in x if y in lbl_stoi.keys()]
            )
            df.drop(["1dAVb", "RBBB", "LBBB", "SB", "AF", "ST"], axis=1, inplace=True)

        reformat_predictions(
            df_cardiologist1, "label_cardiologist1_txt", "label_cardiologist1"
        )
        reformat_predictions(
            df_cardiologist2, "label_cardiologist2_txt", "label_cardiologist2"
        )
        reformat_predictions(df_gold, "label_txt", "label")
        reformat_predictions(
            df_cardiology_residents,
            "label_cardiology_residents_txt",
            "label_cardiology_residents",
        )
        reformat_predictions(
            df_emergency_residents,
            "label_emergency_residents_txt",
            "label_emergency_residents",
        )
        reformat_predictions(
            df_medical_students, "label_medical_students_txt", "label_medical_students"
        )
        ...
        start_idxs = [
            np.where(np.sum(np.abs(sigbufs[i]), axis=1) == 0.0)[0]
            for i in range(len(sigbufs))
        ]  # discard zeros at beginning/end
        start_idxs = [len(a) // 2 for a in start_idxs]

        for id, row in tqdm(list(df.iterrows())):
            data = resample_data(
                sigbufs=sigbufs[id][start_idxs[id] : -start_idxs[id] or None],
                channel_stoi=channel_stoi,
                channel_labels=channel_labels,
                fs=fs,
                target_fs=target_fs,
        ...
        stratified_ids = stratify(
            list(df.apply(lambda row: row["label_txt"] + [row["sex"]], axis=1)),
            lbl_itos + ["male", "female"],
            [1.0 / strat_folds] * strat_folds,
        )
        df["strat_fold"] = -1
        idxs = np.array(df.index.values)
...
def prepare_data_ludb(
    ...
    if recreate_data is True:
            ...
            if extra_segments:
                ...
                for start, end in bgs_ranges:
                    if start > 0 and end < len(x) - 1:
                        if (
                            subdivide
                            and x[start - 1] == label_stoi["p)"]
                            and x[end + 1] == label_stoi["(N"]
                        ):
                            ...
                            subdivide
                            and x[start - 1] == label_stoi["N)"]
                            and x[end + 1] == label_stoi["(t"]
                        ):
                            ...
                            subdivide
                            and x[start - 1] == label_stoi["t)"]
                            and x[end + 1] == label_stoi["(p"]
                        ):
        ...
        for filename in tqdm(list(data_path.glob("**/*.hea"))):
            ...
            anns = [
                wfdb.rdann(str(filename)[:-4], "atr_" + s) for s in header["sig_name"]
            ]
            data_ann = []
            for ann in anns:
                ann_conv, lbl_itos_ann = convert_ann(
                    ann.sample,
                    ann.symbol,
                    len(sigbufs),
                    target_fs,
        ...
        df["label_global"] = df.diag.apply(
            lambda x: [lbl_stoi_global[y] for y in x if y in lbl_stoi_global.keys()]
        )
        df["label_global_txt"] = df.diag.apply(
            lambda x: [y for y in x if y in lbl_stoi_global.keys()]
        )

        lbl_itos = {"global": lbl_itos_global, "annotation": lbl_itos_ann}

        stratified_ids = stratify(
            list(df["label_global_txt"]),
            lbl_itos_global,
            [1.0 / strat_folds] * strat_folds,
        )
        df["strat_fold"] = -1
...
# Cell
def prepare_data_ribeiro_full(
    data_path,
    max_records_per_id_exam=0,
    min_records_per_id_exam=0,
    overwrite_npy_if_exists=False,
    selected_folds=None,
    skip_strat_folds=False,
    subsample=None,
    target_fs=100,
    strat_folds=100,
    channels=12,
    ...
    if recreate_data is True:
        ...
        sex_map = {"M": "male", "F": "female"}
        df["sex"] = df.sex.apply(
            lambda x: sex_map[x] if x in sex_map.keys() else "unknown"
        ).astype("category")

        df["label"] = df[["1dAVb", "RBBB", "LBBB", "SB", "AF", "ST"]].values.tolist()
        df["label"] = df.label.apply(lambda x: list(np.where(np.array(x) == 1.0)[0]))
        lbl_itos = ["1AVB", "RBBB", "LBBB", "SBRAD", "AFIB", "STACH"]
        print(
            "In total",
            len(df),
            "records from more than",
            len(df.id_patient.unique()),
            "patients before filtering.",
        )
        ...
        if max_records_per_id_exam > 0 or min_records_per_id_exam > 0:
            if max_records_per_id_exam > 0:
                df = df[
                    (df.id_record <= max_records_per_id_exam)
                    & (df.id_record >= min_records_per_id_exam)
                ].copy()
            else:
            ...
            print(
                ...
                "records from more than",
                len(df.id_patient.unique()),
                "patients after filtering for max_records_per_id_exam.",
            )
        ...
        if subsample is not None:
            ...
            patient_ids_selected = patient_ids[
                int(subsample[0] * len(patient_ids)) : int(
                    subsample[1] * len(patient_ids)
                )
            ]
            df_labeled = df_labeled[df_labeled.id_patient.isin(patient_ids_selected)]

            df_unlabeled = df[df.missing_label == True].copy()
            df_unlabeled = df_unlabeled.iloc[
                int(subsample[0] * len(df_unlabeled)) : int(
                    subsample[1] * len(df_unlabeled)
                )
            ]
        ...
        if skip_strat_folds is False:
            ...
            df_labeled = df[df.missing_label == False].copy()
            df_labeled["label_sex"] = df_labeled.sex.apply(
                lambda x: len(lbl_itos) + 1 if x == "male" else len(lbl_itos) + 2
            )
            df_labeled["label_age"] = df_labeled.age.apply(
                lambda x: len(lbl_itos) + 3
                if x < 20
                else (
                    len(lbl_itos) + 4
                    if x < 40
                        ...
                        len(lbl_itos) + 5
                        if x < 60
                        else (len(lbl_itos) + 6 if x < 80 else len(lbl_itos) + 7)
                    )
                )
            )
            ...
            df_patients = df_labeled.groupby("id_patient")["labelx"].apply(
                lambda x: list(x)
            )
            patients_ids = list(df_patients.index)
            ...
            stratified_ids = stratify_batched(
                patients_labels,
                range(len(lbl_itos) + 8),
                [1.0 / strat_folds] * strat_folds,
                samples_per_group=patients_num_ecgs,
                verbose=True,
                batch_size=10000,
            ...
            if selected_folds is not None:
                ...
                print(
                    ...
                    "records from more than",
                    len(df.id_patient.unique()),
                    "patients after filtering for selected folds.",
                )
        ...
        def process_single(i, df):
            ...
            if overwrite_npy_if_exists or not (
                (target_root / (filename.stem + ".npy")).exists()
            ):  # recreate npy
                ...
                if target_fs > header["fs"]:
                    print(
                        "Error processing",
                        filename.stem,
                        "sampling frequency insufficient. Will be dropped.",
                    )
                    keep = False
                else:
                    np.save(target_root / (filename.stem + ".npy"), data)
            ...
            if header["base_date"] is not None and header["base_time"] is not None:
                time = datetime.datetime.combine(
                    header["base_date"], header["base_time"]
                )
...
def create_segmentation_mask(
    sig_len, ann_sample, ann_annotation, ann_stoi, ann_unk_id, discard_labels=[""]
):
    """
    ...
    ann_sample = np.array(ann_sample)[
        np.where(np.isin(ann_annotation, discard_labels) == False)[0]
    ]
    ann_annotation = np.array(ann_annotation)[
        np.where(np.isin(ann_annotation, discard_labels) == False)[0]
    ]

    for sym, sta, end in zip(
        ann_annotation, ann_sample, np.concatenate((ann_sample[1:], [sig_len + 1]))
    ):
...
def rescale_segmentation_mask(mask, fs, target_fs):
    ...
    timesteps_new = [
        int(x)
        for x in np.rint(np.array(range(int(len(mask) * factor))) * fs / target_fs)
    ]
...
def rescale_segment_list(segment_list, fs, target_fs):
    factor = target_fs / fs
    return [
        (int(np.round(a * factor)), int(np.round(b * factor)))
...
def get_data_annot_stats(data_path, annotation=True, rhythm=True):
        ...
        if annotation:
            if not Path(str(filename)[:-4] + ".atr").exists():
                ...
                if rhythm:
                    ...
                    meta["rhythm"] = np.unique(
                        np.array(ann.aux_note)[np.array(ann.symbol) == "+"]
                    )
                else:
    ...
    if annotation:
        unique_symbols, unique_symbols_counts = np.unique(
            [item for sublist in list(df_stats.symbol) for item in sublist],
            return_counts=True,
        )
    ...
    if rhythm and annotation:
        unique_rhythms, unique_rhythms_counts = np.unique(
            [item for sublist in list(df_stats.rhythm) for item in sublist],
            return_counts=True,
        )
...
def prepare_physionet_dataset_with_annotations(
    df_stats,
    ann_stoi,
    ann_unk_id,
    dataset_name="mitdb",
    ...
    * segments with uniform rhythm
    ...
    if recreate_data is True:
        ...
        for sample_id, row in tqdm(df_stats.iterrows(), total=len(df_stats)):
            ...
            if rhythm:
                ann_sample = np.array(ann_sample)[
                    np.where(np.array(ann.symbol) == "+")[0]
                ]
                ann_annotation = np.array(ann.aux_note)[
                    np.where(np.array(ann.symbol) == "+")[0]
                ]
            else:
            ...
            if create_segments:  # create segments
                segments, segments_label = create_segment_list(
                    len(sigbufs),
                    ann_sample,
                    ann_annotation,
                    ann_stoi,
                ...
                if drop_unk:
                    segments = np.array(segments)[
                        np.where(np.array(segments_label) != ann_unk_id)
                    ]
                    segments_label = np.array(segments_label)[
                        np.where(np.array(segments_label) != ann_unk_id)
                    ]
                for i, (seg, segl) in enumerate(zip(segments, segments_label)):
                    if seg[1] - seg[0] > 0 and (
                        min_len_segments == 0 or seg[1] - seg[0] >= min_len_segments
                    ):  # discard segments that are too short
                        meta = {
                            "comments": comments,
                            "data": Path(filename.stem + "_" + str(i) + ".npy"),
                            "label": segl,
                            "label_unique": [segl],
                            "start_index": seg[0],
                            "end_index": seg[1],
                        ...
                        np.save(
                            target_root / (filename.stem + "_" + str(i) + ".npy"),
                            data[seg[0] : seg[1]],
                        )
                        metadata.append(meta)
            ...
            meta = {
                "comments": comments,
                "data": Path(filename.stem + ".npy"),
                "label": Path(filename.stem + "_ann.npy"),
                "label_unique": np.unique(mask),
                "patient_id": row["patient_id"]
                if "patient_id" in df_stats.columns
                else sample_id,
            }
        ...
        if create_segments:
            ...
            lbl_unique_single = np.unique(
                [
                    item
                    for sublist in list(df_single["label_unique"])
                    for item in sublist
                ]
            )
        ...
        # global label
        lbl_unique = np.unique(
            [item for sublist in list(df["label_unique"]) for item in sublist]
        )
        ...
        df_patients = (
            (df_single if create_segments else df)
            .groupby("patient_id")["label_unique"]
            .apply(lambda x: list(x))
        )
        patients_ids = list(df_patients.index)
        patients_labels = list(
            df_patients.apply(lambda x: [item for sublist in x for item in sublist])
        )
        patients_num_ecgs = list(df_patients.apply(len))

        stratified_ids = stratify(
            patients_labels,
            lbl_unique_single if create_segments else lbl_unique,
            [1.0 / strat_folds] * strat_folds,
            samples_per_group=patients_num_ecgs,
        )
        stratified_patient_ids = [
        ...
        lbl_itos = (
            [""] * int(1 + max(np.unique(list(ann_stoi.values()))))
            if drop_unk
            else [""] * int(1 + max(np.unique([ann_unk_id] + list(ann_stoi.values()))))
        )
        for k in ann_stoi.keys():
            lbl_itos[ann_stoi[k]] = (
                k if lbl_itos[ann_stoi[k]] == "" else lbl_itos[ann_stoi[k]] + "|" + k
            )
        if drop_unk is False:
            lbl_itos[ann_unk_id] = (
                "UNK" if lbl_itos[ann_unk_id] == "" else lbl_itos[ann_unk_id] + "UNK"
            )
        # add means and std
        dataset_add_mean_col(df, data_folder=target_root)
        dataset_add_std_col(df, data_folder=target_root)
...
def prepare_mitdb(
    data_path,
    ann_stoi=ann_stoi_af,
    ann_unk_id=ann_unk_id_af,
    create_segments=True,
    drop_unk=False,
    target_fs=128,
    strat_folds=5,
    channels=2,
    channel_stoi=channel_stoi_mitdb,
    target_folder=None,
    recreate_data=True,
):
    ...
    print("\n\nProcessing records...")
    return prepare_physionet_dataset_with_annotations(
        df_stats,
        ann_stoi,
        ann_unk_id,
        dataset_name="mitdb",
...
def prepare_afdb(
    ...
    strat_folds=5,
    channels=2,
    channel_stoi=channel_stoi_afdb,
    target_folder=None,
    recreate_data=True,
):
...
def prepare_cpsc21(
    ...
    strat_folds=5,
    channels=2,
    channel_stoi=channel_stoi_cpsc21,
    target_folder=None,
    recreate_data=True,
):
